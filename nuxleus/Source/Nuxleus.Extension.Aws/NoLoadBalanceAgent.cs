...
namespace Nuxleus.Extension.Aws
{
    ...
    public class NoLoadBalanceAgent
    {
        ...
        public void Invoke(string fileName)
        {
            ...
            scope.Begin = () =>
            {
                var tasks = from line in ReadLinesFromFile(fileName)
                            ...
                            select CreatePutAttributesTask("test10", "geonamesid", attributes).AsIAsync();
        ...
        private static PutAttributesTask CreatePutAttributesTask(string domainName, string matchItemName, IEnumerable<Attribute> attributes)
        {
            return new PutAttributesTask
            {
                DomainName = new Domain { Name = domainName },
                Item = new Item { ItemName = attributes.First(a => a.Name == matchItemName).Value, Attribute = attributes.ToList() },
            };
        }
