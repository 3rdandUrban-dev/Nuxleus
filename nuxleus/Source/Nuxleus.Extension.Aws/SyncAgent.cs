            ...
            foreach (string attributeName in attributeNames) {
                ...
                if (attributeValue.Length > 0) {
                    if (attributeValue.Contains(",")) {
                        ...
                        yield return new Attribute(attributeName, attributeValue);
                    }
                }
            }
        }

        private static PutAttributes CreatePutAttributesTasks(string domainName, string matchItemName, IEnumerable<Attribute> attributes) {
            return new PutAttributes { 
                DomainName = domainName, 
                ItemName = attributes.First(a => a.Name == matchItemName).Value, 
                Attribute = attributes.ToList<Attribute>() 
            };
        }
