...
namespace com.amazonaws
{
  public class HttpQueryConnection : IAwsConnection
  {
    ...
    public HttpQueryConnection(string awsAccessKey, string awsSecretKey, 
			       string baseUrl) 
    {
    ...
    private string GetSignature(string canonicalString) 
    {
      ...
      HMACSHA1 signature = new HMACSHA1(ae.GetBytes(awsSecretKey));
      string b64 = 
	Convert.ToBase64String(signature.ComputeHash(ae.GetBytes(canonicalString.ToCharArray())));
      return System.Web.HttpUtility.UrlEncode(b64);
    }
			      ...
			      IDictionary parameters) 
    {
      ...
      finalParameters.Add("AWSAccessKeyId",awsAccessKey);
