//
// MessageQueue.cs: Implementation of the IMessageQueue interface
//
// Author:
...
namespace Nuxleus.Messaging {
  public class MessageQueue : IMessageQueue {
    ...
    private MessageQueue(string ip, int port, string delimiter) {
    ...
    /// <summary>
    /// Gets or sets the time span within which the connection is expected to happen.
    /// </summary>
    ...
    /// <summary>
    /// Sends a message to the queue server.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new MessageQueue instance and initailizes it to the ip address
    /// and port provided. The delimiter defines how each message are separated.
    /// </summary>
    public static IMessageQueue Create(string ip, int port, string delimiter) {
      return new MessageQueue(ip, port, delimiter);
    }

    /// <summary>
    /// Opens up the connection to the queue server.
    /// </summary>
    /// <remark> 
    /// If an error happens during the connection, it won't be raised but
    /// you can trap it setting the Service.Failure handler. The Close() method will
