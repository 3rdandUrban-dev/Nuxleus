...
<xsl:transform version="2.0" 
  ...
  <xsl:function name="profile:is-available">
    <xsl:param name="username" />
    <!-- would check our data store of course... -->
    <xsl:value-of select="true()" />
  </xsl:function>

  <xsl:function name="profile:validate-add">
    <xsl:param name="username" />
    <xsl:param name="openid" />
    <xsl:variable name="errors">
      <profile:errors>
        <xsl:choose>
          <xsl:when test="not($username) and not(profile:is-available($username))">
            <profile:error name="username">*</profile:error>
          </xsl:when>
          <xsl:when test="$username and not(profile:is-available($username))">
            <profile:error name="username">* (this name is already used, try again)</profile:error>
          </xsl:when>
        </xsl:choose>
        <xsl:choose>
          <xsl:when test="not($openid)">
            <profile:error name="openid">*</profile:error>
          </xsl:when>
        </xsl:choose>
      </profile:errors>
    </xsl:variable>
    ...
    <xsl:choose>
      <xsl:when test="$errors/profile:errors/profile:error">
  ...
  <xsl:template match="profile:add-user">
    ...
    <xsl:variable name="session" select="func:resolve-variable(@session-id)" />

    <xsl:if test="profile:validate-add(.)">
