...
<xsl:transform version="2.0" 
    ...
    <xsl:import 
      href="./controller/base.xslt" />
    <xsl:import 
      href="./functions/base.xslt" />
    
    <xsl:param 
      name="current-context" />
    <xsl:param 
      name="response" 
      select="aspnet-context:Response($current-context)" />
    <xsl:param 
      name="request" 
      select="aspnet-context:Request($current-context)" />
    <xsl:param 
      name="server" 
      select="aspnet-context:Server($current-context)" />
    <xsl:param 
      name="session" 
      select="aspnet-context:Session($current-context)" />
    <xsl:param 
      name="timestamp" 
      select="aspnet-context:Timestamp($current-context)" />
    <xsl:param 
      name="aws-config" 
      select="document('./aws.config')/aws" />
    <xsl:param 
      name="aws-default-key-pair-name" 
      select="'primary'" />
    <xsl:param 
      name="aws-default-key-pair-set"
      select="$aws-config/key[@friendly-name = $aws-default-key-pair-name]" />
    <xsl:param 
      name="aws-public-key" 
      select="$aws-default-key-pair-set/public" />
    <xsl:param 
      name="aws-private-key" 
      select="$aws-default-key-pair-set/private" />
    <xsl:variable 
      name="debug"
    ...
    <xsl:variable 
      name="request-uri" 
      select="aspnet-request:Url($request)" />
    <xsl:variable 
      name="browser" 
      select="aspnet-request:Browser($request)" />
    ...
    <xsl:output method="xml" indent="no" encoding="UTF-8" use-character-maps="xml"
