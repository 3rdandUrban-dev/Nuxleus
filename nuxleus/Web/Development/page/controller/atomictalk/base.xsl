...
<xsl:stylesheet
    ...
    exclude-result-prefixes="html exsl my app advice atom head page service resource output form body view menu model collection msxsl doc atompub">

    <xsl:param
        name="closure-token-pre-delimiter"
        select="'|@@'" />
    <xsl:param
        name="closure-token-post-delimiter"
        select="'@@|'" />
    <xsl:param
        name="replace-token-pre-delimiter"
        select="'@@'" />
    <xsl:param
        name="replace-token-post-delimiter"
        select="'@@'" />
    <xsl:param
        name="cond-token-pre-delimiter"
        select="'|$$'" />
    <xsl:param
        name="cond-token-post-delimiter"
        select="'$$|'" />
    <xsl:param
        name="cond-if-token"
        select="'test:'" />
    <xsl:param
        name="cond-then-token"
        select="'IfTrue:'" />
    <xsl:param
        name="cond-else-token"
        select="'IfFalse:'" />
    <xsl:param
        name="system-variable-pre-delimiter"
        select="'%%'" />
    <xsl:param
        name="system-variable-post-delimiter"
        select="'%%'" />
    <xsl:param
        name="parameter-list-pre-delimiter"
        select="'($'" />
    <xsl:param
        name="parameter-list-post-delimiter"
        select="'$)'" />
    <xsl:param
        name="parameter-pre-delimiter"
        select="':'" />
    <xsl:param
        name="parameter-post-delimiter"
        select="':'" />
    <xsl:param
        name="replace-parameter-list-pre-delimiter"
        select="'[$'" />
    <xsl:param
        name="replace-parameter-list-post-delimiter"
        select="'$]'" />
    <xsl:param
        name="replace-parameter-pre-delimiter"
        select="':'" />
    <xsl:param
        name="replace-parameter-post-delimiter"
        select="':'" />
    <xsl:param
        name="parameter-list-delimeter"
        select="','" />
    <xsl:param
        name="parameter-value-assigment-token"
        select="'='" />
    ...
    <xsl:variable
        name="vendor-uri"
        select="system-property('xsl:vendor-uri')" />
    <xsl:variable
        name="page"
        select="/my:session/my:page" />
    <xsl:variable
        name="config"
        select="document($page/page:config/@src)/page:config|$page/page:config" />
    <xsl:variable
        name="browser"
        select="$config/page:browser[@vendor = $vendor]/@replace" />
    <xsl:variable
        name="advice"
        select="$config/page:advice" />
    <xsl:variable
        name="resource"
    ...
    <xsl:variable
        name="view"
        select="document($page/page:view/@src)/page:config|$page/page:view" />
    ...
    <xsl:variable
        name="quote">"</xsl:variable>
    <xsl:variable
        name="squote">'</xsl:variable>
    ...
    <xsl:variable
        name="page.output.head"
        select="document($page/page:output/page:head/@src)/page:head|$page/page:output/page:head" />

    <xsl:variable
        name="page.output.body">
        <xsl:choose>
            <xsl:when
                test="$page/page:output/page:body/@src">
                <xsl:apply-templates
                    select="document($page/page:output/page:body/@src)/page:body" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates
                    select="$page/page:output/page:body" />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    ...
    <xsl:template
        ...
        <html>
            <xsl:apply-templates
                select="$page.output.head" />
            <xsl:copy-of
                select="$page.output.body" />
        </html>
    </xsl:template>
    ...
    <xsl:template
        ...
        <head>
            ...
            <style
                type="text/css">
                <xsl:apply-templates
                    select="head:include[@fileType = 'css']" />
            </style>
            <xsl:apply-templates
                select="head:include[@fileType = 'javascript']" />
        </head>
    </xsl:template>
    ...
    <xsl:template
        match="body:onload|body:onresize">
        <xsl:attribute
            name="{local-name()}">
            <xsl:call-template
                name="replace">
    ...
    <xsl:template
        match="head:include[@fileType = 'css']">
        <xsl:variable
            name="uri">
            <xsl:call-template
                name="resolve-uri">
        ...
        <xsl:value-of
            select="concat('@import ', $quote, $uri, $quote, ';')" />
    </xsl:template>

    <xsl:template
        match="head:include[@fileType = 'javascript' and not(@src)]">
        <script
    ...
    <xsl:template
        match="head:include[@fileType = 'javascript' and @src]">
        <xsl:variable
            name="uri">
            <xsl:call-template
                name="resolve-uri">
    ...
    <xsl:template
        match="advice:*[@compare = 'xsl:vendor']">
        <xsl:value-of
            select="current()[@compare-with = $vendor]/text()" />
    </xsl:template>
    ...
    <xsl:template
        match="layout:view|model:workspace|page:view|view:container|model:collection[not(@src)]">
        <ul
            class="list {@style}"
            id="{@id}">
            <xsl:apply-templates />
    ...
    <xsl:template
        match="doc:feed">
        <xsl:apply-templates
            select="document(@href)/atom:feed/atom:entry">
            <xsl:with-param
                name="cCount"
    ...
    <xsl:template
        match="doc:date">
        <xsl:value-of
            select="document('/date.xml')/date/current" />
    </xsl:template>

    <xsl:template
        match="doc:session.openid">
        <xsl:variable
            select="document(@href)/message/session/@openid"
            name="openid.url" />
        <xsl:choose>
            <xsl:when
                test="$openid.url = 'not-set'">
                <li
                    class="list menu {@style}"
                    id="{@id}">
                    <a
                        href="/login/"
                        title="Log to your amp.fm profile">Login</a>
                </li>
                <li
                    class="list menu {@style}"
                    id="{@id}">
                    <a
                        href="http://openid.amp.fm/signup">Create Account</a>
                </li>
            </xsl:when>
            <xsl:otherwise>
                <li
                    ...
                    id="{@id}">
                    <a
                        href="/service/redirect/?uri=/gatekeeper/logout?uname={$openid.url}&amp;status-code=303"
    ...
    <xsl:template
        match="doc:html[@type = 'myspace-events']">
        <xsl:apply-templates
            select="document(@href)//html:div[@id = current()/@id]" />
    </xsl:template>

    <xsl:template
        match="doc:html[@type = 'external-html']">
        <xsl:apply-templates
            select="document(@href)//body:html" />
    </xsl:template>

    <xsl:template
        match="page:content[@src]">
        <xsl:apply-templates
            select="document(@src)" />
    </xsl:template>
    ...
    <xsl:template
        match="view:item[@src]|collection:item[@src]">
        <li
            class="list {@style}"
            id="{@id}">
            <xsl:apply-templates
    ...
    <xsl:template
        match="view:item[not(@src)]|collection:item[not(@src)]">
        <li
            class="list menu {@style}"
            id="{@id}">
            <xsl:apply-templates />
    ...
    <xsl:template
        ...
        <ul
            ...
            id="{@id}">
            <xsl:apply-templates
                select="document(@src)/view:menu" />
        </ul>
    </xsl:template>

    <xsl:template
        match="view:menu[not(@src)]">
        <ul
            class="list menu {@style}"
            id="{@id}">
            <xsl:apply-templates />
    ...
    <xsl:template
        match="atom:summary" />
    <xsl:template
        match="atom:published" />
    <xsl:template
        match="atom:updated" />
    <xsl:template
        match="atom:generator" />
    <xsl:template
        match="atom:id" />
    <xsl:template
        match="atom:category" />
    ...
    <xsl:template
        ...
        <p
            ...
            <xsl:copy-of
                select="substring(., 1, $cCount)" /> ... [<a
                href="{../atom:link[@rel = 'self']/@href}">more</a>] </p>
    </xsl:template>
    ...
    <xsl:template
        ...
        <xsl:choose>
            <xsl:when
                test="contains($nString, $closure-token-pre-delimiter)">
                <xsl:variable
                    name="name"
            ...
            <xsl:when
                test="contains($nString, $replace-token-pre-delimiter)">
                <xsl:variable
                    name="name"
                ...
                <xsl:variable
                    name="replace-with">
                    <xsl:apply-templates
                        select="$advice/advice:*[local-name() = $name]" />
                </xsl:variable>
                <xsl:call-template
                    name="replace">
                    <xsl:with-param
    ...
    <xsl:template
        ...
        <xsl:choose>
            <xsl:when
                test="contains($string, $cond-token-pre-delimiter)">
                <xsl:variable
                    name="sString"
                ...
                <xsl:variable
                    name="else"
                    select="substring-after($conditional, $cond-else-token)" />
                <xsl:variable
                    name="nString">
                    <xsl:choose>
                        <xsl:when
                            ...
                            <xsl:value-of
                                select="concat($pre-cond, $replace-string, $post-cond)" />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:variable
                                name="replace-string">
    ...
    <xsl:template
        ...
        <xsl:variable
            name="nReplace-var-string">
            <xsl:call-template
                name="replace">
                <xsl:with-param
                    name="string"
                    select="$replace-var-string" />
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable
            name="name"
