...
namespace Mvp.Xml.Common.XPath
{
	...
	public class XPathNavigatorReader : XmlTextReader, IXmlSerializable, IXPathNavigable
	{
		...
		public XPathNavigatorReader(XPathNavigator navigator) : this(navigator, false)
		{
		}
		...
		private ArrayList OrderedAttributes
		{
			...
			{ 
				...
				// Get a properly positioned navigator
				...
				_orderedattributes = new ArrayList();
				for (bool attr = nav.MoveToFirstAttribute(); attr; attr = nav.MoveToNextAttribute())
				{
					_orderedattributes.Add(new string[] { nav.LocalName, nav.NamespaceURI });
				}

				// Add all namespace attributes declared at the current node.
				nav.MoveToParent();
				for (bool ns = nav.MoveToFirstNamespace(XPathNamespaceScope.Local); ns; 
					ns = nav.MoveToNextNamespace(XPathNamespaceScope.Local))
				{
					// Don't add the xmlns declaration itself.
					if (nav.Value != XmlNamespaces.Xml)
						_orderedattributes.Add(new string[] { nav.LocalName, XmlNamespaces.XmlNs });
				}
		...
		/// <summary>
		/// Whether the reader should continue even if the current element has ended or is empty.
		/// </summary>
		private bool AllowFragment
		{
			get 
			{ 
				return _fragment || 
		...
		{
			XPathNavigator current = _navigator.Clone();
		...
		{
			string ns = _navigator.GetNamespace(prefix);
		...
		public override bool MoveToAttribute(string name, string ns) 
		{
			...
			bool moved = _navigator.MoveToAttribute(name, ns);
		...
		public override bool MoveToNextAttribute() 
		{
			...
			if (_navigator.NodeType == XPathNodeType.Attribute)
			{
				...
				if (!moved)
				{
					...
					XPathNavigator lastattr = _navigator.Clone();
					_navigator.MoveToParent();
		...
		public override bool Read() 
		{
			...
			else
			{
				...
				if (_navigator.IsSamePosition(_original))
				{
					...
					{
						// Reposition original pointer.
						// If this is false, the following code will check ending conditions.
						_original.MoveToNext();
					}
				}
				...
				{
					// Check whether the document was completely empty.
					if (_navigator.IsSamePosition(_original) &&
						_navigator.IsEmptyElement) 
					{
						_eof = true;
						_state = ReadState.EndOfFile;
						return false;
