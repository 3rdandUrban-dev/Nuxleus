...
namespace Mvp.Xml.Common.XPath
{
	...
	/// (XmlDocument, XPathDocument or any other IXPathNavigable store) thus
	/// providing alternative way to select nodes directly from built index table 
	/// and not searhing the tree. This allows drastically decrease selection time
	/// on preindexed selections.
	/// </summary>
	/// <remarks>Author: Oleg Tkachenko, oleg@tkachenko.com
	/// Contributors: Daniel Cazzulino, kzu.net@gmail.com
	/// <para>See http://www.tkachenko.com/blog/archives/000194.html</para>
	/// </remarks>
	public class IndexingXPathNavigator : XPathNavigator
	{
		...
		private class XPathNavigatorIndex 
		{
			...
			private Type arrayIteratorType;
			...
			public XPathNavigatorIndex() 
			{
				...
				arrayIteratorType = systemXml.GetType("System.Xml.XPath.XPathArrayIterator");
			}
			...
			{          
				...
				ArrayList indexedNodes = null;
				if (keyValue is XPathNodeIterator) 
				{
					XPathNodeIterator nodes = keyValue as XPathNodeIterator;
					while (nodes.MoveNext()) 
					{        
						ArrayList chunkOfIndexedNodes = (ArrayList)index[nodes.Current.Value];          
						if (chunkOfIndexedNodes != null) 
						{
							if (indexedNodes == null)
								indexedNodes = new ArrayList();
							indexedNodes.AddRange(chunkOfIndexedNodes);        
						}
					}
				} 
				else 
				{        
					indexedNodes = (ArrayList)index[keyValue.ToString()];
				}      
				if (indexedNodes == null)
					indexedNodes = new ArrayList(0);            

				return (XPathNodeIterator) Activator.CreateInstance( arrayIteratorType, 
					BindingFlags.Instance | BindingFlags.Public | 
					BindingFlags.CreateInstance, null, new object[]{indexedNodes}, null ); 
			}
			...
			{
				//Get slot
				ArrayList indexedNodes = (ArrayList)index[key];
				if (indexedNodes == null) 
				{
					indexedNodes = new ArrayList();
