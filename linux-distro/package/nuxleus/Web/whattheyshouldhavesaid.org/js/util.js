...
{
    return outputString.replace(lt, "<") .replace(gt, ">") .replace(amp, "&");
...
function MoreResults(layer, resultsArray, places, hasMore, veErrorMessage)
{
    ...
    if(hasMore)
    {
        var r = "<a href='javascript:FindLoc(parseInt(10));'>" +
               "Click for More Results</a> (Showing ".concat(count).concat(' of '.concat(layer.GetShapeCount())).concat(' )');
    ...
    else
    {
        ...
        window.setTimeout("$('#results').fadeOut('slow');", 2000);
    }
}

function outputVars(path) {
  var perspective, timespan, view, location, filters;
  $.log("Path: ".concat(path));
  var view = path.split('/');
  var viewLength = view.length;
  for (var i = 0; i < viewLength; i++){
    var current = view[i].split(':');
    switch(current[0]){
      case 'perspective':
        perspective = (view[i]) ? view[i] : 'home';
        break;
      case 'timespan':
        timespan = (view[i]) ? view[i] : 'timespan:today';
        break;
      case 'view':
        viewspan = (view[i]) ? view[i] : 'view:newspaper';
        break;
      case 'location':
        location = view[i]
        break;
  ...
  $.log("Perspective: ".concat(perspective));
  $.log("Timespan: ".concat(timespan));
  $.log("View: ".concat(viewspan));
  $.log("Location: ".concat(location));
  $.log("Filters: ".concat(filters)); 
  return new Array(perspective, timespan, viewspan, location, filters);
}